...
| strict.strictEntryPointCoverage               | `false` | Will throw an error if a shared external cannot be served coherently — i.e. a remote declares a secondary entrypoint the shared basis's build does not contain ("entrypoint tearing"). If the value is false, the entrypoint is served from the consuming remote's own build (self-fill, with a warning) so nothing is lost, unless `profile.scopeUncoveredEntrypoints` is enabled, in which case the uncovered copy is scoped instead. See [Entrypoint coverage and tearing](./version-resolver.md#entrypoint-coverage-and-tearing).                                             |
...
| profile.scopeUncoveredEntrypoints         | `false`     | When enabled, a remote copy whose secondary entrypoints the shared basis cannot cover is split out into a "scoped" version and serves its whole `entries` bunch from its own build, instead of tearing the package across two builds. Sharing continues for the copies the basis does cover. Has no effect when `strict.strictEntryPointCoverage` is set, which throws instead.                                                    |
...
| feature.useAutoExternalPooling | `false` | When enabled, shared externals are grouped into pools by their npm scope (`@framework/core`, `@framework/common` → pool `framework`) and no remote is allowed to draw a coupled family from builds that disagree: it either takes the whole family from builds that agree, or serves the whole family from its own build. Members may still be served from different remotes. Buys coherence at a possible cost in downloads, never a reduction. Unscoped packages are not auto-pooled; a remote can also opt a specific external into a pool with a `pool` tag on its shared external regardless of this flag. See [Dependency Pooling](./version-resolver.md#dependency-pooling). |
