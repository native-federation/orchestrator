...
describe('createManifestProvider', () => {
  ...
  describe('provide', () => {
    ...
    it('should handle fetch errors properly', async () => {
      ...
      await expect(manifestProvider.provide(manifestUrl)).rejects.toEqual(
        new NFError("Fetch of 'http://bad.service/manifest.json' returned 404 - Not Found")
      );
    });
    ...
    it('should handle JSON parsing errors for manifest', async () => {
      ...
      await expect(manifestProvider.provide(manifestUrl)).rejects.toEqual(
        new NFError("Fetch of 'http://bad.service/manifest.json' returned Invalid JSON")
      );
    });
  });
});
