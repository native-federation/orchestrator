...
/**
 * Storage routine shared by the init pipeline (process-remote-entries, step 2) and the
 * dynamic initRemoteEntry pipeline (update-cache, step 8). Only the resolution of a
 * singleton external differs between the two and is injected per call — the pipelines
 * must not be merged, see docs/version-resolver.md "Dynamic Init".
 */
