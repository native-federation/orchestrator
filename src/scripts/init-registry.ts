import { createRegistry } from 'lib/registry/setup-registry';
import type { NFEventRegistry } from '../lib/registry/event-registry.contract';
import type { NFEventRegistryOptions } from '../lib/registry/registry.contract';
...
(function (): void {
  ...
  const options: NFEventRegistryOptions = {
    // Maximum number of different event types to keep in the registry.
    maxStreams: script?.dataset?.maxStreams ? Number(script.dataset.maxStreams) : undefined,

    // Maximum number of events to keep in the history for each event type.
    maxEvents: script?.dataset?.maxEvents ? Number(script.dataset.maxEvents) : undefined,
